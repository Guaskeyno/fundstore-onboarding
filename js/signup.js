/* Fundstore onboarding prototype · sign-up journey
   signup.xhtml (method) → signup-details.xhtml (name, surname, email) → signup-password.xhtml → dossier.xhtml
   Each page runs only the parts whose markup it contains. */
(() => {
  const { $, $$, wait, user, saveUser, go, flag } = window.FS;

  flag('seenSignup', true);
  ...
  /* ---------- Every sign-up page: links that are out of scope ---------- */
  const google = $('[data-action="google"]');
  ...
  if (detailsForm) {
    ...
    detailsForm.addEventListener('submit', async e => {
      ...
      await wait(800);
      go('signup-password.xhtml');
    });
  }

  /* ---------- signup-password.xhtml (Figma screens 3 + 4) ---------- */
  ...
  if (pwForm) {
    ...
    wait(250).then(() => toast('Email verificata'));
  }
})();
